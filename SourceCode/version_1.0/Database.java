...
public class Database{
    ...
    public String lastLoginPassword="请输入密码";
    ...
    public void saveFileToDisk()
    {
        try(ObjectOutputStream oos =new ObjectOutputStream(new FileOutputStream(DATA_LOCATION))){
            ...
            oos.writeObject(this.isSelected);
        }catch(IOException e){
            e.printStackTrace();
        }
    }
    ...
    public void loadFileToProgram()
    {
        ...
        if(file.exists()){
            try(ObjectInputStream ois =new ObjectInputStream(new FileInputStream(DATA_LOCATION))){
                ...
                this.isSelected=(boolean)ois.readObject();
            }catch(IOException e){
                e.printStackTrace();
    ...
    public void setLastLoginInformation(String name,String password,boolean rememberPassword)
    {
        lastLoginName=name;
        if(rememberPassword){
